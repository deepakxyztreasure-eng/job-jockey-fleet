...
const map: Record<string, { label: string; cls: string }> = {
  ...
  completion_requested: { label: "Completion Requested", cls: "bg-priority/15 text-priority" },
  rejected:             { label: "Rejected",             cls: "bg-destructive/15 text-destructive" },
  issue:                { label: "Issue",                cls: "bg-destructive/15 text-destructive" },
...
export function PaymentBadge({ status }: { status: string }) {
  ...
  return <Badge className={`${m[status] ?? m.pending} border-0 font-medium capitalize`}>{status}</Badge>;
}
